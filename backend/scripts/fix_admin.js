...
async function run() {
  ...
  try {
    const [admins] = await pool.query('SELECT id, username, role FROM users WHERE username="admin" OR role="admin"')
    ...
    for (const admin of admins) {
      console.log(`Setting password for ${admin.username} (ID: ${admin.id}) to Admin123!...`)
